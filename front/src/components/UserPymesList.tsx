                ...
                {tableHeaders.map((header) => (
                  <th key={header} className="p-3 min-w-40">
                    <div className="flex items-center gap-1">
                      {header}{" "}
                      <MdKeyboardArrowDown className="mt-1 w-6 h-6 cursor-pointer" />
                    </div>
                  </th>
                ))}
                ...
                {pymesByUser &&
                  pymesByUser.payload.map((pyme: GetPymeResponse) => {
                    const canRequestCredit =
                      pyme.statusCredit !== "Enviado" && pyme.hasDocuments;
                    return (
                      <tr
                        key={pyme.id}
                      ...
                      >
                        ...
                        <td className="p-3">
                          {pyme.hasDocuments ? (
                            ...
                            <span
                              onClick={() =>
                                navigate(
                                  `/Dashboard/RegistroDocumentosPyme/${pyme.id}`
                                )
                              }
                        ...
                        <td className="p-3">
                          ...
                          ${
                            pyme.statusCredit === "Activo"
                              ? "bg-[#12b92f] text-white"
